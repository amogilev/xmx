...
package com.gilecode.ucfg.impl;

import com.gilecode.ucfg.*;
...
public class UpdatingIniConfigLoader implements IUpdatingConfigLoader<Ini> {
	...
	private String mergeComments(String curComment, String[] defaultCommentLines) {
		...
		for (String curLine : curCommentLines) {
			if (curLine.startsWith(IniSettings.AUTO_COMMENT_PREFIX)) {
				// well-formatted auto-comment line
				curLine = curLine.substring(IniSettings.AUTO_COMMENT_PREFIX.length()).trim();
			} else if (curLine.equals(IniSettings.AUTO_EMPTY_LINE)) {
				// well-formatted auto-comment empty line
				curLine = "";
			} else if (curLine.startsWith(IniSettings.AUTO_PREFIX)) {
	...
	private String generateAutoComments(OptionDescription...optionDescriptions) {
		...
		for (OptionDescription optDesc : optionDescriptions) {
			sb.append(IniSettings.AUTO_EMPTY_LINE).append(lineSeparator); // separate options with empty comment lines
			for (String commentLine : optDesc.getComments()) {
				sb.append(IniSettings.AUTO_COMMENT_PREFIX).append(commentLine).append(lineSeparator);
			}
			sb.append(IniSettings.AUTO_PREFIX).append(' ').append(optDesc.getName())
	...
	private String generateComment(List<String> autoComments, List<String> manualComments) {
		...
		if (autoComments != null) {
			for (String line : autoComments) {
				if (line.isEmpty()) {
					sb.append(IniSettings.AUTO_EMPTY_LINE).append(lineSeparator);
				} else {
					sb.append(IniSettings.AUTO_COMMENT_PREFIX).append(line);
		...
		for (Section section : ini.values()) {
			if (prevSection != null && prevSection.getComment(ENDSECT_ANCHOR) == null) {
				...
				if (sectionComment != null &&
						...
						sectionComment.contains(IniSettings.AUTO_PREFIX + " ")) {
	...
	private static void parseCommentLines(List<SectionLineInfo> result,
		...
		try {
			while ((line = br.readLine()) != null) {
				if (line.startsWith(IniSettings.AUTO_PREFIX)) {
					if (line.startsWith(IniSettings.AUTO_COMMENT_PREFIX)) {
						line = line.substring(IniSettings.AUTO_COMMENT_PREFIX.length());
					...
					line = line.substring(IniSettings.AUTO_PREFIX.length());
		...
		for (int i = from; i <= to; i++) {
			...
			if (kindsSet.contains(line.kind) && line instanceof CommentLine) {
				...
				if (cl.commentLine.isEmpty()) {
					sb.append(IniSettings.AUTO_EMPTY_LINE);
				} else if (cl.kind == SectionLineInfo.Kind.AUTOCOMMENT) {
					sb.append(IniSettings.AUTO_COMMENT_PREFIX);
				} else if (cl.kind == SectionLineInfo.Kind.DEFOPTION) {
					sb.append(IniSettings.AUTO_PREFIX).append(' ');
				}
