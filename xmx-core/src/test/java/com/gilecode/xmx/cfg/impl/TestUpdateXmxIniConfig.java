...
import com.gilecode.ucfg.impl.IniSettings;
...
import static com.gilecode.xmx.cfg.impl.ConfigDefaults.*;
...
public class TestUpdateXmxIniConfig {
	...
	public void testUpdateNotRequiredWithChangedOrder() throws Exception {
		...
		for (String line : section.getSectionComments()) {
			sectionLines.add("#" + (line.isEmpty() ? IniSettings.AUTO_EMPTY_LINE : IniSettings.AUTO_COMMENT_PREFIX + line));
		}
	...
	private List<String> makeIniLinesWithSectionOverride(String sectionName, List<String> sectionLines) {
		...
		for (String line : FILE_COMMENTS) {
			result.add("#" + (line.isEmpty() ? IniSettings.AUTO_EMPTY_LINE : IniSettings.AUTO_COMMENT_PREFIX + line));
		}
		result.add("");
	...
	private List<String> makeOptionLinesWithOverrideValue(OptionDescription option,
		...
		result.add("#" + IniSettings.AUTO_EMPTY_LINE);
		for (String line : option.getComments()) {
			result.add("#" + IniSettings.AUTO_COMMENT_PREFIX + line);
		}
		if (includeDefaultCommentedValue) {
			result.add("#" + IniSettings.AUTO_PREFIX + " " + option.getName() +
